...
const unsigned char BSL430_Vendor_Version = 0xFF;
#else
#pragma DATA_SECTION(BSL430_Vendor_Version, ".BSL430_VERSION_VENDOR");
...
void main(void)
{
    ...
    if (!_calledFromApp && BSL430_isAppValid())
...
void interpretCommand()
{
    ...
    switch (command)
    {
        ...
        case LOAD_PC:                     // Load PC
            ...
            } else if (!BSL430_isAppValid()) {
                sendMessage(BSL_INVALID_APP);
            } else {
                sendMessage(SUCCESSFUL_OPERATION);
                BSL430_callAddress(addr);
            }
            break;
#endif
        /*-------------------------------------------------------------------*/
