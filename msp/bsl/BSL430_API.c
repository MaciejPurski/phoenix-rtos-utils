...
{
    if (LockedStatus == UNLOCKED && BSL430_isAppValid())
    {
#ifdef RAM_BASED_BSL
        flushBuffer();
#endif
...
int BSL430_isLocked(void)
{
    return (LockedStatus != UNLOCKED);
}

int BSL430_isAppValid(void)
{
    char *mb = (char*)0x4400; /* Location of magic bytes */

    if (mb[0] == 0xde && mb[1] == 0xad && mb[2] == 0xbe && mb[3] == 0xef)
        return 1;

    return 0;
}

void BSL430_callAppResetHandler(void)
{
    unsigned *reset_vector = (unsigned*)0xfffe;
    void (*reset_handler)() = (void (*)())*reset_vector;
