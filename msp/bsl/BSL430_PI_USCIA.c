...
char PI_receivePacket()
{
  ...
  while( RX_StatusFlags == RX_PACKET_ONGOING )
  {
    ...
    res = receiveByte(&dataByte);               // get another byte from host
    if (res < 0)
        continue;
...
void init_USCI( char baud_rate )
{
  ...
  switch ( baud_rate )
  {
    ...
    break;
    case BAUD_19200:
      UCA0BRW = 208;                            // 4MHz 19200 (see User's Guide)
      UCA0MCTL = UCBRS_6 + UCBRF_0;             // Modulation
    break;
    case BAUD_38400:
      UCA0BRW = 138;                            // 4MHz 38400 (see User's Guide)
      UCA0MCTL = UCBRS_3 + UCBRF_0;             // Modulation
    break;
    case BAUD_57600:
      UCA0BRW = 69;                            // 4MHz 57600 (see User's Guide)
      UCA0MCTL = UCBRS_7 + UCBRF_0;             // Modulation
    break;
    case BAUD_115200:
      UCA0BRW = 34;                            // 4MHz 115200 (see User's Guide)
